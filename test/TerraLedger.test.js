...
describe("TerraLedger", function () {
    ...
    describe("Property Registration", function () {
        it("Should allow the owner to register a new property with full details", async function () {
            const { terraLedger, owner, accountOne } = await deployTerraLedgerFixture();

            // Register a property to 'accountOne' with area and type
            await terraLedger.connect(owner).registerProperty(
                accountOne.address,
                "123 Main St",
                "1500 sq ft",
                "Residential"
            ...
            expect(property.area).to.equal("1500 sq ft");
        ...
        it("Should fail if a non-owner tries to register a property", async function () {
            ...
            await expect(
                terraLedger.connect(accountOne).registerProperty(
                    accountOne.address,
                    "456 Side St",
                    "3000 sq ft",
                    "Commercial"
